...
<?php
   ...
   echo "<tr style='border:1px black solid; '>";
     ...
     echo "<th ><h4><em>Pharmacy id</em></h4></th>";
    echo  "<th><h4><em>Pharmacy name</em></h4></th>";
   ...
   $sql="select purchase.pid,pharmacy.name as names,customer.id,customer.fname,medicines.mid,medicines.name ,purchase.quantity,purchase.price,purchase.method,date(purchase.date) as date from purchase INNER JOIN customer on purchase.cid=customer.id INNER JOIN medicines on medicines.mid=purchase.mid INNER join pharmacy on purchase.pid=pharmacy.pid order by date DESC";
   $result=mysqli_query($conn,$sql);
   while($row=mysqli_fetch_assoc($result)){
     $count++;
     echo "<tr style='border:1px black solid; height:100px;'>";
          echo  "<td><h4>".$row['pid']."</h4></td>";
     echo  "<td><h4>".$row['names']."</h4></td>";
...
<?php
   ...
   $sql="select customer.id ,customer.fname,medicines.mid,medicines.name ,purchase.quantity,purchase.price,purchase.method,date(purchase.date) as date from purchase INNER JOIN customer on purchase.cid=customer.id INNER JOIN medicines on medicines.mid=purchase.mid  where pid = 2 and date(purchase.date) ='$date'";
   $result=mysqli_query($conn,$sql);
   while($row=mysqli_fetch_assoc($result)){
     $count++;
     echo "<tr style='border:1px black solid; height:100px;'>";
